...
const cqcToAircalcDeckBuilder = cqc => {
  console.log(cqc)
  const fleets = _.compact(cqc.fleets)
  const airbases = _.compact(cqc.airbase)
  const dFleets = _.fromPairs(
    fleets.map((fleet,ind) => {
      const dShips = _.fromPairs(
        _.compact(fleet.ships).map((ship,sInd) => {
  ...
  const dAirbases = _.fromPairs(
    airbases.map((ab, ind) => {
      const items = _.fromPairs(_.compact(ab.slots).map((sq,ind1) => {
        const {mstId: id, ace: mas, imp: rf} = sq
        return [`i${ind1+1}`, {id, mas, rf}]
      }))
      return [`a${ind+1}`, {mode: 1, items}]
    })
  )
